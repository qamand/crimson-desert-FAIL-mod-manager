...
export type ModType = 'data' | 'asi' | 'language';
...
export interface IFileSystem {
  ...
  loadConfig(): Promise<{ gamePath?: string; modsPath?: string } | null>;
  saveConfig(data: { gamePath: string; modsPath: string }): Promise<void>;
}
...
export interface IModEngine {
  installMod(archivePath: string, modsPath?: string): Promise<{ modId: string; entry: ModEntry }>;
  removeMod(modId: string, modsPath?: string): Promise<boolean>;
  listMods(modsPath?: string): Promise<ModEntry[]>;
...
export interface RegisterAllHandlersServices {
  ...
  store: { gamePath: string; modsPath: string };
}