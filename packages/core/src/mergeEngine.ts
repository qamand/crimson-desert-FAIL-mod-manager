...
import { IFileSystem, ILogger, ISecurityGuard, IModEngine, ModEntry } from '@modmanager/types';
import * as path from 'path';

function getModType(fileName: string): 'data' | 'asi' | 'language' {
...
export class ModEngine implements IModEngine {
  ...
  async installMod(archivePath: string, modsPath?: string): Promise<{ modId: string; entry: ModEntry }> {
    ...
    if (ext === '.zip') {
      // Архив распаковываем в папку с именем архива (без расширения)
      ...
      // Одиночный файл копируем прямо в mods/
  ...
  async listMods(modsPath?: string): Promise<ModEntry[]> {
    ...
    // Только файлы в корне папки модов
    ...
    // Также можно добавить папки (от архивов), но пока пропускаем
